...
@objc(Iosvideo) class Iosvideo : CDVPlugin , RTCClientDelegate  , RTCEAGLVideoViewDelegate{
    ...
    func configureVideoClient() {
        ...
        let iceServers = RTCIceServer.init(urlStrings: [self.stunServer], username: self.username, credential: self.password)
        print(iceServers)
    ...
    func setViewOfVideo(){
        self.callLocalView = RTCEAGLVideoView.init(frame: CGRect(x: 0, y: 0, width: SCREEN_WIDTH, height: SCREEN_HEIGHT))
        self.callRemoteView.contentMode = .center
        self.callLocalView.contentMode = .scaleAspectFit
        //本地视频翻转
        self.callLocalView.transform=CGAffineTransform.init(scaleX: -1, y: 1)
        self.callRemoteView.addSubview(callLocalView)
    }
    func removeStream(){
        self.minStr = ""
        self.hrStr = ""
...
extension Iosvideo{
    func addCallerView(image:String , isCallComing : Bool, appName : String , doctorName : String , status : String , rejectStr : String , acceptStr : String) {
        ...
        callImageView.backgroundColor = #colorLiteral(red: 0.1792264283, green: 0.1641918719, blue: 0.2932648063, alpha: 1)//UIColor.white
        callRemoteView.addSubview(callImageView)
        upperNavView = UIView.init(frame: CGRect.init(x: 0, y: 0, width: SCREEN_WIDTH, height: 110))
        upperNavView.alpha = 0.8
        ...
        currentStatusLabel = UILabel.init(frame: CGRect.init(x: 10, y: 72, width: SCREEN_WIDTH-20, height: 22))
        ...
        self.acceptBtn = UIImageView.init(frame: CGRect.init(x: SCREEN_WIDTH/2-30+SCREEN_WIDTH/4, y: SCREEN_HEIGHT-100, width: 60, height: 60))
        self.speakerBtn = UIImageView.init(frame: CGRect.init(x: SCREEN_WIDTH/2-30+SCREEN_WIDTH/4, y: SCREEN_HEIGHT-100, width: 60, height: 60))
        ...
        if isCallComing == true{//被呼叫
            rejectBtn = UIImageView.init(frame: CGRect.init(x: SCREEN_WIDTH/2-30-SCREEN_WIDTH/4, y: SCREEN_HEIGHT-100, width: 60, height: 60))
            self.acceptBtn.isHidden = false
        }else{//呼叫方
            rejectBtn = UIImageView.init(frame: CGRect.init(x: SCREEN_WIDTH/2-30, y: SCREEN_HEIGHT-100, width: 60, height: 60))
    ...
    @objc internal func acceptCall(gestureRecognizer: UITapGestureRecognizer) {
        print("acceptCall")
        if self.isVideoCall{
            self.rejectBtn.frame = CGRect.init(x: SCREEN_WIDTH/2-30, y: SCREEN_HEIGHT-100, width: 60, height: 60)
        }else{
            UIView.animate(withDuration: 0.33) {
                self.rejectBtn.frame = CGRect.init(x: SCREEN_WIDTH/2-30-SCREEN_WIDTH/4, y: SCREEN_HEIGHT-100, width: 60, height: 60)
            }
            self.speakerBtn.isHidden = false
        }
        self.acceptBtn.isHidden = true
        if self.isCallComing{
            ...
            self.currentStatusLabel.text = "通话中"
        }
        self.videoClient?.makeOffer()
    }
    ...
    func callJS(data: String) {
        let javaScript = "cordova.plugin.Iosvideo.callbackResult(\(data))"
        DispatchQueue.main.async {
            if let appl = UIApplication.shared.delegate as? CDVAppDelegate{
...
extension Iosvideo  {
        ...
        if let type = value["type"] as? String{
            ...
            case IonicTypes.incomingCall.rawValue:
                ...
                if let data = value["data"] as? [String : Any] {
                    ...
                    self.username=setting["username"] as! String
                    self.password=setting["password"] as! String
                    if callType == "A"{
                    ...
                    self.addCallerView(image: img, isCallComing: isCallComing, appName: appName, doctorName: name, status: "请求通话...", rejectStr: rec, acceptStr: acc)
                    self.configureVideoClient()
                }
            ...
            case IonicTypes.sdp_callResponse.rawValue://接收answer信息
                if let data = value["data"] as? String {
                    print(data)
                    self.caseOnAnswer(sdpAns: data)
                }
            case IonicTypes.sdp_startCommunication.rawValue://接收offer并创建answer信息
                if let data = value["data"] as? String {
                    print(data)
                    self.createAnswerForOfferReceived(sdpAns: data)
                }
            case IonicTypes.call.rawValue:
                if let data = value["data"] as? [String : Any] {
                    ...
                    self.addCallerView(image: img, isCallComing: isCallComing, appName: appName, doctorName: name, status: "正在请求通话...", rejectStr: rec, acceptStr: acc)
                    self.configureVideoClient()
    ...
    func caseOnAnswer(sdpAns : String) -> Void {
        ...
        if(self.isVideoCall){
            //将本地视频上移
            UIView.animate(withDuration: 0.33) {
                self.callLocalView.frame = CGRect.init(x: SCREEN_WIDTH-130, y: 80, width: 120, height: 180)
            }
            //隐藏状态栏
            self.upperNavView.isHidden=true
        }
    }
